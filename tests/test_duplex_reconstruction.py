...
class TestDerivedSequence:
    ...
    def test_the_two_strands_differ(self, genome):
        ...
        path, _ = genome
        df = pd.DataFrame([_align_row("chr1", 100, 140, "+"), _align_row("chr1", 100, 140, "-")])
        out = reconstruct(df, path)
    ...
    def test_non_default_index_is_preserved(self, genome):
        path, seqs = genome
        df = pd.DataFrame(
            [_align_row("chr1", 0, 40, "+"), _align_row("chr2", 0, 40, "+")], index=["x", "y"]
        )
        out = reconstruct(df, path)
...
class TestBedtoolsParity:
    ...
    def test_derived_sequences_match_bedtools(self, genome):
        ...
        for _ in range(200):
            ...
            rows.append(
                _align_row(chrom, start, start + random.randint(20, 45), random.choice("+-"))
            )
