...
class TestParseIntervalLabel:
    def test_round_trips_a_label(self):
        assert parse_interval_label("chrI:1807-2169(-)") == ("chrI", 1807, 2169, "-")
...
class TestExtractionIsOneBasedInclusive:
    def test_an_exon_spans_end_minus_start_plus_one_bases(self, genome):
        ...
        seqs = get_exon_seqs(gtf([(101, 200, "+", "T1")]), path, transcript_id="T1")
    ...
    def test_an_intron_lies_between_the_exons(self, genome):
        path, sequence = genome
        seqs = get_intron_seqs(gtf([(101, 200, "+", "T1"), (301, 400, "+", "T1")]), path, "T1")
    ...
    def test_adjacent_exons_yield_no_intron(self, genome):
        path, _ = genome
        seqs = get_intron_seqs(gtf([(101, 200, "+", "T1"), (201, 300, "+", "T1")]), path, "T1")

        assert seqs == {}

    def test_a_single_base_intron_is_one_base(self, genome):
        path, sequence = genome
        seqs = get_intron_seqs(gtf([(101, 200, "+", "T1"), (202, 300, "+", "T1")]), path, "T1")
    ...
    def test_a_spliced_transcript_concatenates_its_exons(self, genome):
        path, sequence = genome
        spliced = get_spliced_seq(gtf([(101, 200, "+", "T1"), (301, 400, "+", "T1")]), path, "T1")
...
class TestLocalToGenomic:
    def test_a_plus_strand_offset_is_added_to_the_interval_start(self):
        ...
        assert local_to_genomic("chr1:101-200(+)", 0, 30) == ("chr1", 100, 130, "+")

    def test_a_plus_strand_probe_at_the_interval_end(self):
        assert local_to_genomic("chr1:101-200(+)", 70, 100) == ("chr1", 170, 200, "+")
    ...
    def test_a_minus_strand_offset_is_mirrored(self):
        ...
        assert local_to_genomic("chr1:101-200(-)", 0, 30) == ("chr1", 170, 200, "-")

    def test_a_minus_strand_probe_at_the_far_end(self):
        assert local_to_genomic("chr1:101-200(-)", 70, 100) == ("chr1", 100, 130, "-")

    def test_the_interval_length_is_inclusive(self):
        # a 1-base interval must map local [0,1) onto exactly one base
        assert local_to_genomic("chr1:500-500(+)", 0, 1) == ("chr1", 499, 500, "+")
        assert local_to_genomic("chr1:500-500(-)", 0, 1) == ("chr1", 499, 500, "-")

    def test_coordinates_stay_ascending_on_both_strands(self):
        for strand in ("+", "-"):
            _, start, stop, _ = local_to_genomic(f"chr1:101-200({strand})", 10, 40)
            assert start < stop
...
class TestMinedProbesMapBackToTheReference:
    ...
    def test_a_probe_mined_from_an_interval_is_found_at_its_coordinates(self, genome, strand):
        path, sequence = genome
        label = f"chr1:101-900({strand})"

        seqs = get_exon_seqs(gtf([(101, 900, strand, "T1")]), path, transcript_id="T1")
        ...
        for _, local_start, local_stop, probe_seq, _ in probes[:20]:
            _, start, stop, _ = local_to_genomic(label, local_start, local_stop)
...
class TestFeatureSelectorContract:
    ...
    def test_giving_both_selectors_is_rejected(self):
        ...
        with pytest.raises(ValueError, match="both"):
            _select_features(gtf([(101, 200, "+", "T1")]), transcript_id="T1", gene_id="G1")
    ...
    def test_one_selector_is_accepted(self):
        ...
        assert len(_select_features(gtf([(101, 200, "+", "T1")]), transcript_id="T1")) == 1
