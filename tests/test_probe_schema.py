...
class TestVersioning:
    def test_a_manifest_without_a_version_is_refused(self):
        with pytest.raises(schema.SchemaError, match="no schema_version"):
            schema.validate_manifest({"created": "x", "n_probes": 0, "columns": [], "stages": []})
...
class TestJsonIo:
    ...
    def probe_set(self, example_probes):
        ...
        probe_set._record(
            "screen", params={"max_kmer": 5}, n_in=len(probe_set), n_out=len(probe_set)
        )
        return probe_set
    ...
    def test_round_trip_preserves_the_probes(self, probe_set, tmp_path):
        ...
        pd.testing.assert_frame_equal(
            back.df.reset_index(drop=True), probe_set.df.reset_index(drop=True), check_dtype=False
        )
