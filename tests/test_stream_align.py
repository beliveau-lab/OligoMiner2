...
needs_bowtie2 = pytest.mark.skipif(shutil.which("bowtie2") is None, reason="bowtie2 not installed")
...
def probes(genome):
    ...
    _, seq = genome
    rows = [{"seqid": f"p{i}", "probe_seq": seq[i * 400 : i * 400 + 36]} for i in range(20)]
...
class TestCommandBuilder:
    ...
    def test_preset_overrides_seed_parameters(self):
        cmd = build_bowtie2_cmd(
            "/tmp/idx", input_file="/tmp/p.fastq", preset={"D": 20, "R": 3, "N": 1, "L": 20}
        )
...
class TestStreamingAlignment:
    ...
    def test_a_bad_index_raises_rather_than_writing_an_empty_bed(self, probes, tmp_path):
        ...
        with pytest.raises(ExternalCommandFailed):
            align_to_bed(probes, str(tmp_path / "absent_index"), tmp_path / "out.bed", threads=1)
    ...
    def test_streaming_matches_the_buffered_path(self, probes, bt2_index, tmp_path):
        ...
        sam = bowtie_align(
            bt2_index, input_file=str(fastq), threads=1, k=10, no_unal=True, reorder=True
        )
        buffered = sorted(line for line in bam_to_bed(bam_data=sam).strip().split("\n") if line)
