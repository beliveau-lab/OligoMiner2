...
class TestProcessAlignments:
    ...
    def test_derived_sequences_are_uppercased_by_default(self, reference):
        ...
        df = process_alignments(sam_data=SAM_DATA, ref_fasta=reference).set_index("seqid")
        assert df.loc["probe2", "derived_seq"].isupper()

    def test_case_is_preserved_when_asked(self, reference):
        df = process_alignments(sam_data=SAM_DATA, ref_fasta=reference, to_upper=False).set_index(
            "seqid"
        )
...
class TestFromBam:
    def test_a_bam_gives_the_same_frame_as_its_sam(self, tmp_path):
        ...
        with open(bam, "wb") as handle:
            subprocess.run(["samtools", "view", "-bS", str(sam)], stdout=handle, check=True)
...
class TestDerivedSequenceAlignment:
    ...
    def test_a_short_sequence_list_raises(self, reference, monkeypatch):
        ...
        mod = importlib.import_module("oligominer.specificity.alignment.process_alignments")
