...
class TestShellPipeline:
    def test_stages_are_chained(self):
        result = ShellPipeline().add(["printf", "b\na\nc\n"]).add(["sort"]).run()

        assert result.split() == ["a", "b", "c"]

    def test_add_multi_chains_in_order(self):
        result = ShellPipeline().add_multi(["printf", "one\ntwo\n"], ["wc", "-l"]).run()
    ...
    def test_input_reaches_the_first_stage(self):
        result = ShellPipeline().add(["cat"]).add(["tr", "a-z", "A-Z"]).run(input_data="shout")
    ...
    def test_output_file_returns_none_and_writes(self, tmp_path):
        ...
        result = (
            ShellPipeline().add(["printf", "x\ny\n"]).add(["wc", "-l"]).run(output_file=str(path))
        )
