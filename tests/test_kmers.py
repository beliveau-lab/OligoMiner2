...
class TestDispatch:
    ...
    @pytest.mark.skipif(have_jellyfish() is not None, reason="jellyfish is installed")
...
class TestMetadataGuards:
    ...
    def test_canonicality_mismatch_raises(self, numpy_index):
        with pytest.raises(KmerIndexError, match="is_canonical"):
            max_kmer(numpy_index, ["ACGTACGTACGTACGT"], k=K, backend="numpy", expect_canonical=True)

    def test_matching_canonicality_is_accepted(self, numpy_index):
        counts = max_kmer(
            numpy_index, ["ACGTACGTACGTACGT"], k=K, backend="numpy", expect_canonical=False
        )
        assert len(counts) == 1
    ...
    def test_numpy_backend_refuses_canonical_builds(self, tmp_path, reference):
        ...
        with pytest.raises(KmerIndexError, match="forward strand"):
            build_index(str(fasta), str(tmp_path / "c.npz"), k=K, backend="numpy", canonical=True)
...
class TestConcurrentBuildScratch:
    ...
    def test_concurrent_builds_sharing_a_tmp_dir_agree_with_a_lone_build(self, tmp_path, reference):
        ...
        def build(name):
            ...
            KmerIndex.build(str(fasta), k=K, min_count=2, tmp_dir=str(shared)).save(str(out))
