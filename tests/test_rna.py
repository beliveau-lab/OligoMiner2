...
def annotation():
    ...
    """
    rows = []
    for transcript, spans in (
        ("tx1", [(101, 200), (301, 400), (501, 600)]),
...
class TestJunctionWindows:
    ...
    def test_the_window_joins_sequence_that_is_not_genomically_adjacent(self, genome, annotation):
...
class TestDiscriminatingRegions:
    def test_the_unique_exon_of_each_isoform_is_found(self, annotation):
        ...
        by_transcript = {
            row["transcript_id"]: (row["start"], row["end"]) for _, row in regions.iterrows()
        }
...
class TestBuildTranscriptome:
    ...
    def test_records_are_named_by_transcript_id(self, genome, annotation, tmp_path):
        ...
        names = [line[1:].strip() for line in out.read_text().splitlines() if line.startswith(">")]
        assert names == ["tx1", "tx2"]

    def test_sequence_length_matches_the_spliced_length(self, genome, annotation, tmp_path):
        path, _ = genome
        out = tmp_path / "tx.fa"
        build_transcriptome(annotation, str(path), str(out))
    ...
    def test_the_transcriptome_can_be_indexed_and_searched(self, genome, annotation, tmp_path):
        ...
        hits = [line for line in aligned.stdout.splitlines() if line and not line.startswith("@")]
