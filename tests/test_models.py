...
class TestLoadingAndScoring:
    @pytest.mark.parametrize(
        ...
        [
            ...
            "ps-xgb",
            "om1-lda",
            pytest.param("duplex-BiLSTM", marks=pytest.mark.needs_torch),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "name",
        ["physics-xgb", "ps-xgb", pytest.param("duplex-BiLSTM", marks=pytest.mark.needs_torch)],
    )
    ...
    def test_xgb_output_is_a_probability_not_a_logit(self, duplex_df):
        ...
        raw = model.model.predict(features_fast.enc_om2_fast(frame).values.astype(np.float32))
...
class TestEmptyFrame:
    ...
    def empty_frame(self):
        empty = pd.DataFrame(columns=["probe_seq", "derived_seq", "align_cigar", "align_score"])
...
class TestConditionResponse:
    ...
    def _mean_by_temperature(self, name, duplex_df, temperatures):
        ...
        return [
            model.predict(build_duplex_frame(duplex_df, celsius=t, sodium=0.39)).mean()
...
class TestEmptyDuplexFrame:
    ...
    def test_an_empty_table_builds_an_empty_frame(self):
        empty = pd.DataFrame(columns=["probe_seq", "derived_seq", "align_cigar"])
