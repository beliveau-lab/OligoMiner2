...
class TestBuildTranscriptome:
    ...
    def test_a_transcript_is_its_exons_spliced(self, annotated_genome, tmp_path):
...
class TestDesignAgainstTheTranscriptome:
    def test_a_probe_in_a_shared_exon_hits_both_transcripts(self, annotated_genome, tmp_path):
        genome, gtf, _ = annotated_genome
        ...
        subprocess.run(["bowtie2-build", "-q", str(transcriptome), index], check=True)
        ...
        df = process_alignments(sam_data=aligned.stdout, ref_fasta=str(transcriptome))
    ...
    def test_the_duplex_frame_builds_from_transcriptome_alignments(
        ...
        lookup = {f"{seq_id}:{start}-{stop}": probe for seq_id, start, stop, probe, _ in probes}
